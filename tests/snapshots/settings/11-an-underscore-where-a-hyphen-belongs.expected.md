# An underscore where a hyphen belongs

<!-- From 11-an-underscore-where-a-hyphen-belongs.md. -->
<!-- Generated. Do not edit; UPDATE_EXPECT=1 cargo test -->

## stdout

```
```

## stderr

```
findopera: ./findopera.toml is not valid:
    TOML parse error at line 2, column 1
      |
    2 | require_variants = true
      | ^^^^^^^^^^^^^^^^
    unknown field `require_variants`, expected one of `template`, `destination`, `link`, `require-variants`, `follow-links`, `ignore`, `index`
```

## Destination

```tree
```

## Requests

```
```

## Exit

```
2
```
